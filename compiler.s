...
%macro ADJUST_THE_STACK 1  ; %1 real size of param
	; backup reg
	push rcx
	push r9
	push r10
	...
	mov rcx, PARAM_COUNT                ; rcx <- size of param from stack, in here we will consider as number of args
	lea r10, [rbp+(4+rcx)*WORD_SIZE]    ; r10 <- pointer to the arg[n]=magic
	sub r10, 8                          ; r10 <- pointer to the arg[n-1]
	mov r11, SOB_NIL_ADDRESS            ; r11 <- will be the cdr in the list we build, so at the start its nil
	sub rcx, %1                         ; rcx <- number of elements need to be in the optinal param (the list)
%%create_pair_loop:
	cmp rcx, 0
	jl %%create_pair_loop_end   	    ; we finish building the list
	mov r8, [r10]               	    ; r8 <- arg[n] (any n)
	MAKE_PAIR(r9, r8, r11)      		; make pair r9 <- (r10 . r11)
	mov r11, r9                		    ; r11 <- r9 (the pair) for next iter
	sub r10, WORD_SIZE        		    ; r10 <- pointer dec by 8 
	dec rcx                    		    ; rcx <- dec by one the number of elements need to be in the optinal param (the list)
	jmp %%create_pair_loop
%%create_pair_loop_end:
	add r10, WORD_SIZE                  ; r10 <- pointer inc by 8 
	jmp .finish
.finish:
	; restore reg
	mov  qword[r10] , r11         ;put the list or nil we create in the right place
	pop r11
	pop r10
	pop r9
	pop rcx
...
%macro SHIFT_FRAME 1; %1 = size of frame
	push rax
	mov rax, PARAM_COUNT
	add rax, 5
	mov r11, rax
...
%macro DYN_SHIFT_FRAME 1; %1 = new size of frame
	push r11
	push rax
	mov rcx, %1
	mov rax, PARAM_COUNT
...
%define STR_DATA_PRT(r) r+WORD_SIZE+TYPE_SIZE
...
%define MAKE_LITERAL_CLOSURE(r, env, body)  \
...
; %define CAR LOWER_DATA car is already defined
